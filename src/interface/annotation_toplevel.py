...
class AnnotationToplevel (object):
    ...
    def register(self, widget: tkinter.Widget, content: str, level: int = 3):
        widget.bind("<Motion>", lambda *_: self.update_position(), True)
        widget.bind("<Enter>", lambda *_: self.deiconify_content(content, level), True)
        widget.bind("<Leave>", lambda *_: self.withdraw(), True)
